...
#  exec [host] control:part change:[add|del|change|replace] device:[left|right]
#   command:"[command]"
# Check success:
...
def print_usage(x):
    ...
    x.p.msg(" - [host] control:part change:[add|del|change|replace] "
            "device:[left|right] command:\"[command]\"\n", False)
    x.p.msg("\n check success:\n", False)
...
def netem_part_handler(x, conf, args, dic):
    try:
        ...
        if side == "left":
            dic["device"] = conf\
                .get_middle_box_left_iface_name(dic["host_name"])
        elif side == "right":
            dic["device"] = conf\
                .get_middle_box_right_iface_name(dic["host_name"])
        else:
            x.p.err("error: wrong usage\n")
            return False
        # command
